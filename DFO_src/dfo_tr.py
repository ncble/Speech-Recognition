...
class params:
	...
	"""
	def __init__(self):
		self.init_delta = 5.0 #1.0  # initial delta (i.e. trust region radius)
		self.tol_delta = 1e-9 #1e-10  # smallest delta to stop
		self.max_delta = 100.0  # max possible delta
		# TR radius adjustment parameters
		self.gamma1 = 0.9 # DFO6: 0.9 #0.8  # radius shrink factor
		self.gamma2 = 1.7 #1.5  # radius expansion factor

		self.eta0 = 0.001 #0.0   # step acceptance test (pred/ared) threshold
		self.eta1 = 0.25  # (pred/ared) level to shrink radius
		self.eta2 = 0.75  # (pred/ared) level to expand radius

		self.tol_f = 1e-12 #1e-15  # threshold for abs(fprev - fnew)- used to stop
		...
		self.maxfev = 90#50000  # maximum number of iterations
...
def _build_initial_sample(x_initial, delta, options=None):
	...
	nY = Y.shape[1] # Number of points  Lu  Y.shape = (n, nY)
	return (Y, nY)
...
def constraint_shift(x, constraints = np.array([-5,5])+np.zeros(2)[:,None]):
	"""
	Example:
	
	x = array([[ 6],
			  [-3]])
	constraints = array([[-5.,  5.],
						   [-5.,  5.]])
	

	"""
	# for axis, constraint in enumerate(constraints):
	x_origin = x
	x = x.reshape(-1,1)
	assert constraints.shape[1] == 2

	outside_the_box = np.any(np.prod(x<constraints, axis = 1)) or np.any(np.prod(x>constraints, axis = 1))

	if outside_the_box:
		## Code provisoire !!!!! TODO TODO
		a = x[0,0]
		b = x[1,0]
		if abs(a)>abs(b):
			if a>0:
				return np.array([5, 5.*b/a])[:, None]
			else:
				return np.array([-5, -5.*b/a])[:, None]
		else:
			if b>0:
				return np.array([5.*a/b, 5])[:, None]
			else:
				return np.array([-5.*a/b, -5])[:, None]
	else:
		return x_origin
...
def dfo_tr(bb_func, x_initial, options=None):
	...
	# Added by Lu
	# dirname = "./dfo_data_lu/{}/".format(socket.gethostname())
	...
	for i in range(nY):
		# f_values[i] = bb_func(Y[:, i].reshape(n, 1)) # Commented by Lu
	...
	while True: # LU
	# for _ in tqdm(range(max_iter+1)): # LU
		success = 0
		...
		if normg <= tol_norm_g:
			print("break due to normg <= tol_norm_g")
			break
		if delta < tol_delta:
			print("break due to delta < tol_delta")
			break
		...
		if abs(val) < tol_f:
			print("break due to abs(val) < tol_f")
			break
		if iteration > max_iter:
			print("break due to iteration > max_iter")
			break
		...
		# ftrue = bb_func(xtrial) # Commented by Lu
		xtrial = constraint_shift(xtrial) # Added by Lu [IM] TODO
	...
	if verbosity:
		...
		print ("Norm of the gradient of the model is {}.".format(normg))
		#print ('***************Final Report**************\n')
