...
def batch_loop(solver, suite, observer, budget,
	...
	for problem_index, problem in enumerate(suite):
		# if ("f002" not in problem.id): # Lu modified
		#     continue
		#####
		...
		short_info.add_evals(problem.evaluations, runs) # + problem.evaluations_constraints Lu
...
def coco_optimize(solver, fun, max_evals, max_runs=1e9):
	...
	for restarts in range(int(max_runs)):
		remaining_evals = max_evals - fun.evaluations# - fun.evaluations_constraints Lu
		...
		elif solver.__name__ in ("dfo_tr",):
			options = {"maxfev": 100000}  # default results in too many stops due to tol_f
			# options = {"tol_f": 1e-10}
			#options = {"maxfev": 1000, "init_delta": 20,
			#           "tol_delta": 1e-25, "tol_f": 1e-26, "tol_norm_g": 1e-15,
...
##############################################################################
		...
		if fun.evaluations >= max_evals or fun.final_target_hit: # + fun.evaluations_constraints
			break
...
max_runs = 1e9#1e9 LU  # number of (almost) independent trials per problem instance
...
suite_instance = "instances: 1" # "year:2016"
suite_options = "dimensions: 10 function_indices:10"  # "dimensions: 2,3,5,10,20 "  # if 40 is not desired
...
observer_options = ObserverOptions({  # is (inherited from) a dictionary
					'algorithm_info': "DFO-TR", # CHANGE/INCOMMENT THIS!
